...
var wipeMessage = function wipeMessage() {
  $('#game-message').text('');
};

var gameAlreadyOver = function gameAlreadyOver() {
  $('#game-message').text('The game is already over. Press New Game to play again.');
};

var alreadyPlayed = function alreadyPlayed() {
  $('#game-message').text('That square has already been played in');
};

var newPlayer = function newPlayer(mark) {
...
var onUpdateSuccess = function onUpdateSuccess(data, domElement, mark, won) {
...
var onAjaxFailure = function onAjaxFailure(error) {
  $('#game-message').text('The server cannot be reached.');
};
...
var gameObj = __webpack_require__(15);
...
var newGame = function newGame(event) {
  ...
  api.createGame().then(function (data) {
    ...
    var blankBoard = ['', '', '', '', '', '', '', '', ''];
    var currGame = new gameObj.Game(blankBoard);
...
var displayGames = function displayGames(event) {
  event.preventDefault();
  api.getCompletedGames().then(ui.onDisplayGames).catch(ui.onAjaxFailure);
};
...
var arrayTo2D = function arrayTo2D(array) {
  var arrayBoard = [];
  var boardWidth = 3;
...
var boardToHTML = function boardToHTML(currGame) {
  var array = arrayTo2D(currGame.cells);
  ...
  for (var row = 0; row < array.length; row++) {
    ...
    for (var col = 0; col < array[row].length; col++) {
      ...
      var textNode = document.createTextNode(array[row][col]);
      // the number in a 9-element list
      var listNum = 3 * row + col;
...
var gameFxns = __webpack_require__(16);
var ui = __webpack_require__(3);
var api = __webpack_require__(4);
...
Game.prototype.move = function (domElement) {
  ...
  api.updateMove(JSON.stringify(apiData)).then(function (data) {
    ui.onUpdateSuccess(data, domElement, _this.currPlayerMark, _this.won);
  }).then(function () {
    if (_this.over === false) {
      _this.changePlayer();
      ui.newPlayer(_this.currPlayerMark);
    }
...
var checkWinner = function checkWinner(array, mark) {
  // check the possible crosses for three in a row
  if (threeInARow(array, mark, 0, 1, 2) || threeInARow(array, mark, 3, 4, 5) || threeInARow(array, mark, 6, 7, 8) ||
  // up & down
  threeInARow(array, mark, 0, 3, 6) || threeInARow(array, mark, 1, 4, 7) || threeInARow(array, mark, 2, 5, 8) ||
  // diagonals
...
var threeInARow = function threeInARow(array, mark, id1, id2, id3) {
  if (array[id1] !== mark) {
    return false;
...
var countPlayedCells = function countPlayedCells(array) {
  var count = 0;
...
module.exports = {
  ...
  countPlayedCells: countPlayedCells
};
